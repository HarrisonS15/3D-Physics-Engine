...
int run() {
    ...
    Triangle obj = Triangle(Vertex(0,0,0), Vertex(0,1,0), Vertex(1,0,0));
    float* verts = obj.getVerticesAsArray();
     // set up vertex data (and buffer(s)) and configure vertex attributes
    // ------------------------------------------------------------------
    ...
    // idea, instead of hard coding 9, use 3* vertices.size
    glBufferData(GL_ARRAY_BUFFER, obj.getNumVertices() * 3 * sizeof(float), verts, GL_STATIC_DRAW);
